                ...
                switch (ping.command()) {
                    ...
                    case TASK: Thread newthread = null;
                               if(ping.task().type() == Task.Type.MAPRED) {
                                   newthread =
				       new Thread(new Mapper(ping.task()));
                               } else { // REDUCE
                                   newthread =
				       new Thread(new Reducer(ping.task()));
                               }
                               newthread.start();
                               break;
                    case RECEIVE: Ping.receiveFile(master, ping.filepath());
                                  break;
    ...
    public static class Mapper implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                while(fis.available() >= task.recordlen()) {
                ...
                Task redtask = new Task(task.jobnum(), task.type(),
                    task.mapreduce(), task.recordlen(), outfilepath,
                    task.outfile());
